...
set serveroutput on
/*
delete from cust_rinta partition(period_$TGL)
where nd in(select distinct a.nd from cust_rinta partition(period_$TGL) a,
ten_nd b where a.nd=b.nd and b.ten_id=$TEN)
/
commit
/
...
/
 ...
 declare
   ...
   SELECT hfc.ncli,
          ...
          FROM  h_f_conso_agregat@DBL_FBIP.REGRESS.RDBMS.DEV.US.ORACLE.COM hfc,RINTA_ADD_$TGL b
	      where hfc.ncli=b.ncli and hfc.ndos=b.ndos and hfc.an_fact=$THN and hfc.per_fact=$BLN
