...
/
 declare
   ...
   SELECT     HE.AN_FACT, 
              ...
              he.mnt_taxe_fact,
				      ...
				      he.cprod,
				      he.mnt_solde_fact
            FROM  h_expedition@odsnas HE,client@odsnas cl,dossier@odsnas dos,p_autocom@odsnas pa
 ...
 loop
 w_c := w_c+1;
 IF   i.METERAI >= 1000000 THEN
         i.METERAI := '6000';
      ELSIF i.METERAI >= 250000  THEN
         i.METERAI := '3000';
              ELSE
         i.METERAI := '0';
            END IF;
 insert into H_EXPEDITION partition(period_$TGL)
                                ( AN_FACT, 
                                 PER_FACT,
	                               NFACT,
                                 NCLI,
...
/
  ...
  from F_CONSO_AGREGAT_BCK@DBL_FBIP.REGRESS.RDBMS.DEV.US.ORACLE.COM a,dossier@DBL_FBIP.REGRESS.RDBMS.DEV.US.ORACLE.COM b,p_autocom@DBL_FBIP.REGRESS.RDBMS.DEV.US.ORACLE.COM
 c
  where   a.an_fact= $THN
  		 and a.per_fact= $BLN
  		 and a.groupe_fact = 'A'
