...
#/******************************************************************/
   ...
   echo "                                                                     "
   echo "          +---------------------------------------------------------+"
   echo "          |######### <|  Loading data Proses H_EXPEDITION  |> ######|"
   echo "          +---------------------------------------------------------+"
   tput cup 19 0
...
cektab=`sqlplus -s c2bi/telkom123@pangrango <<_OFF
      ...
      set feed off
      set serverout on
      select  count(distinct per_fact) from h_expedition@ODSNAS 
...
#echo "tes_ok $thnbln"
   ...
   echo  "+-----------------------------------------------------------------------------------+"
   echo -e '\E[37;44m'"\033[1m|######### <|  Data H_EXPEDITION $thnbln@ODSNAS belum tersedia  |> ######|\033[0m"
   echo  "+-----------------------------------------------------------------------------------+"
fi
TGL2=`date`
