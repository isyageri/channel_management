...
#/******************************************************************/
   ...
   echo "                                                                    "
   echo "          +---------------------------------------------------------+"
   echo "          |######### <|  Loading Proses penambahan data    |> ######|"
   echo "          +---------------------------------------------------------+"
   tput cup 19 0
...
CEK=` sqlplus -s  c2bi/telkom123@jepunrac<<_OFF
          ...
          set feed off
          set serverout on
	       select substr(max(table_name),20,6)  from all_tables where table_name like 'TREMS_REKAP_REGALL_$thnbln%'
	       ;
	        exit
		_OFF
		`         
