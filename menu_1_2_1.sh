...
CEK=` sqlplus -s  c2bi/telkom123@jepunrac<<_OFF
          ...
          set feed off
          set serverout on
	       select count(*) from all_tables where table_name='TREMS_REKAP_REGALL_$TGL'
	       ;
	        exit
		_OFF
		`
...
sqlplus  c2bi/telkom123@jepunrac<<_OFF

prompt"---------------gat data TREMS_REKAP_REGALL-----------------"
...
---/
ALTER TABLE cust_rinta TRUNCATE PARTITION PERIOD_$TGL
/
commit
/
DECLARE
 ...
 select  an_fact,
         ...
         h.nd,
         cprod,
         to_number(nvl(sum(abonemen),0)) abonemen,
         to_number(nvl(sum(diskon),0)) mnt_tck_c,
         to_number(nvl(sum(ppn),0)) ppn,
         to_number(nvl(sum(MATERAI),0)) meterai,
         to_number(nvl(sum(grand_total),0)) GRAND_TOTAL,
         to_number(nvl(sum(lokal),0)) lokal,
         to_number(nvl(sum(sljj),0)) sljj,
         to_number(nvl(sum(stb),0)) stb,
         to_number(nvl(sum(japati),0)) japati,
         to_number(nvl(sum(sli_telkom),0)) sli007,
         to_number(nvl(sum(sli_non_telkom),0)) sli001,
         to_number(nvl(sum(TGLOBAL),0)) SLI_017,
         to_number(nvl(sum(interlokal),0)) interlokal,
         to_number(nvl(sum(TENI),0)) TELKOMNET_INSTAN,
         to_number(nvl(sum(TELKOMSAVE),0)) telkomsave,
         to_number(nvl(sum(NON_JASTEL),0)) non_jastel,
         to_number(nvl(sum(fitur),0))+to_number(nvl(sum(KONTEN),0)) konten,
         to_number(nvl(sum(TOTAL),0)) TOTAL from TREMS_REKAP_REGALL_$TGL h,(select distinct nd from ten_nd) b
where h.nd=b.nd group by an_fact,
         per_fact,
         h.ncli,
         h.ndos,
         h.nd,
...
declare
   cursor c1 is
 select ncli,ndos,nom from cust_rinta partition(period_$TGL3) ;
 w_c number(9) := 0;
 begin
   for i in c1
 loop
...
cursor c1 is select a.ncli,a.nom from client@DBL_FBIP.REGRESS.RDBMS.DEV.US.ORACLE.COM a,
	          cust_rinta partition(period_$TGL) b where a.ncli=b.ncli and b.nom is null;
w_c number(15) := 0;
begin
    for i in c1
loop
w_c := w_c+1;
 UPDATE  cust_rinta  partition(period_$TGL) 
            SET NOM=i.NOM
           WHERE NCLI=i.NCLI  AND AN_FACT=$THN AND PER_FACT=$BLN
           and NOM is null;
